...
def get_leaf_representation(node, tag_list, label, tokenize = None):
    ...
    if tokenize is None:
        ...
        words = tokenize.convert_ids_to_tokens(tokenize.encode(node.string, max_length=1024, truncation=True)[1:-1])
    for word in words:
...
def parse(filenames, language = "All"):
    ...
    if language == "Japanese":
        tokenize = BertTokenizer.from_pretrained('cl-tohoku/bert-base-japanese-whole-word-masking')
